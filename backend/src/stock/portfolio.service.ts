...
export class PortfolioService {
    ...
    async getGroupedTransactions(uid: number): Promise<TransactionsGrouped> {
        ...
        const groupedTransactions =
            transactions.reduce((container: TransactionsGrouped, currentTransaction: TransactionSplitAdjusted) => {
    ...
    agregateTransactions(transactions: TransactionSplitAdjusted[]): TransactionAgregationData {
        const agregatedTransactions = transactions.reduce((agregatedTransactions: TransactionAgregationData, transaction) => {
            if (transaction.buy) {
    ...
    async createGainAndSplitAdjustedStock(sid: number, transactionAgregationData: TransactionAgregationData): Promise<StockGainAndSplitAdjusted> {
        ...
        const gainAbsolute = transactionAgregationData.moneyRecievedFromSales - transactionAgregationData.moneyInvestedInStock
            + (transactionAgregationData.amountAfterSplit * currentPrice);
        const gainPercentage = (gainAbsolute - transactionAgregationData.moneyInvestedInStock) / transactionAgregationData.moneyInvestedInStock * 100;
    ...
    async getPortfolioData(uid: number): Promise<Portfolio> {
        ...
        for (const stockId in transactionAgregationCollection) {
            ...
            const gainAndSplitAdjustedStock = await this.createGainAndSplitAdjustedStock(Number(stockId), transactionAgregationCollection[stockId]);
